...
A [Semaphore group](/V4-beta/glossary/#group) contains [identity commitments](/V4-beta/glossary/#commitment) of group members.
...
Semaphore groups are [Merkle trees](/V4-beta/glossary#merkle-tree), and it is therefore possible to calculate the Merkle proof of a group member (i.e., tree leaf) by passing the index of the member to the `generateMerkleProof`. For example:
...
Semaphore provides [`Semaphore.sol`](https://github.com/semaphore-protocol/semaphore/blob/main/packages/contracts/contracts/Semaphore.sol), a contract designed for managing on-chain groups ([deployed](/V4-beta/deployed-contracts) on major testnets).
