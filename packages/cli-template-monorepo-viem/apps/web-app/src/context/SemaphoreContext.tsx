...
export const SemaphoreContextProvider: React.FC<ProviderProps> = ({ children }) => {
    ...
    const SemaphoreViemInstance = useMemo(() => {
        return new SemaphoreViem(ethereumNetwork, {
            address: process.env.NEXT_PUBLIC_SEMAPHORE_CONTRACT_ADDRESS
        })
    }, [])

    const refreshUsers = useCallback(async (): Promise<void> => {
        const members = await SemaphoreViemInstance.getGroupMembers(process.env.NEXT_PUBLIC_GROUP_ID as string)
    ...
    const refreshFeedback = useCallback(async (): Promise<void> => {
        const proofs = await SemaphoreViemInstance.getGroupValidatedProofs(process.env.NEXT_PUBLIC_GROUP_ID as string)
