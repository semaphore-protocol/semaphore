...
describe("CrossChain", () => {
  ...
  const signersA = ganacheAccounts.map(
    (ganacheAccount) => new Wallet(ganacheAccount["secretKey"], providerA)
  )
  const signersB = ganacheAccounts.map(
    (ganacheAccount) => new Wallet(ganacheAccount["secretKey"], providerB)
  )
  ...
  before(async () => {
    ...
    accounts = await Promise.all(
      signersA.map((signer: Signer) => signer.getAddress())
    )
    ...
    const transactionA = contractA
      .connect(signersA[1])
      .createGroup(groupId, treeDepth, accounts[1], maxEdges)
    await expect(transactionA)
      .to.emit(contractA, "GroupCreated")
    ...
    const transactionB = contractB
      .connect(signersB[1])
      .createGroup(groupId, treeDepth, accounts[1], maxEdges)
    await expect(transactionB)
      .to.emit(contractB, "GroupCreated")
      .withArgs(groupId, treeDepth, 0)
    await expect(transactionB)
    ...
    await expect(tx1a)
      ...
      .withArgs(groupId, membersA[0], groupA.root)
    ...
    await expect(tx2a)
      .to.emit(contractA, "MemberAdded")
      .withArgs(groupId, membersA[1], groupA.root)
    ...
    await expect(tx3a)
      .to.emit(contractA, "MemberAdded")
      .withArgs(groupId, membersA[2], groupA.root)
    allRootsA.push(BigNumber.from(groupA.root).toHexString())
    ...
    await expect(tx1b)
      ...
      .withArgs(groupId, membersB[0], groupB.root)
  ...
  describe("# CrossChainVerify", () => {
    ...
    it("Should verify local proof chainA", async () => {
      ...
      await expect(transaction)
        .to.emit(contractA, "ProofVerified")
