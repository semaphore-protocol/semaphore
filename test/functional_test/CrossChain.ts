...
import { constants, Signer, utils, BigNumber, providers, Wallet, ContractTransaction } from "ethers"
...
describe("2-sided CrossChain tests", () => {
  ...
  const AddMembersAndVerifyEvents = async (semaphore: Semaphore, membersToAdd: BigNumber[], linkedGroup: LinkedGroup) => {
  ...
  let ganacheAdmin = new ethers.Wallet('c0d375903fd6f6ad3edafc2c5428900c0757ce1da10e5dd864fe387b32b91d7e', ganacheProvider2);
  ...
  before(async () => {
    ...
    console.log(`Semaphore 1 has been deployed to ${semaphore1.contract.address} with signer: `, semaphore1.signer.provider.connection)
    ...
    console.log(`Semaphore 2 has been deployed to ${semaphore2.contract.address} with signer: `, semaphore2.signer.provider.connection)
    ...
    const transactionA = semaphore1
      .createGroup(groupId, treeDepth, hardhatAdminAddr, maxEdges)
    ...
    const transactionB = semaphore2
      .createGroup(groupId, treeDepth, ganacheAdminAddr, maxEdges)
    ...
    historicalRootsA = await AddMembersAndVerifyEvents(semaphore1, membersA.slice(0, 3).map((bigint) => BigNumber.from(bigint)), groupA)

    historicalRootsB = await AddMembersAndVerifyEvents(semaphore2, membersB.slice(0, 3).map((bigint) => BigNumber.from(bigint)), groupB)
  ...
  describe("# CrossChainVerifyRoots", () => {
    ...
    it("Should not verify if updateEdge has not been called", async () => {
      ...
      const transaction1 = semaphore1.contract
        .verifyRoots(groupId, createRootsBytes(roots1))
      await expect(transaction1).to.be.revertedWith(
        "Not initialized edges must be set to 0"
      )

      const transaction2 = semaphore2.contract
        .verifyRoots(groupId, createRootsBytes(roots2))
    ...
    it("Should verify external root after updateEdge", async () => {
      ...
      const is_valid = await semaphore2
        .verifyRoots(groupId, createRootsBytes(roots))

      expect(is_valid).to.equal(true)
    })
    it("Should not verify invalid order of roots", async () => {
      const transaction1 = semaphore1
        .verifyRoots(groupId, createRootsBytes(roots2))
      await expect(transaction1).to.be.revertedWith(
        "Cannot find your merkle root"
      )

      const transaction2 = semaphore2
        .verifyRoots(groupId, createRootsBytes(roots1))
      await expect(transaction2).to.be.revertedWith(
        "Cannot find your merkle root"
      )
